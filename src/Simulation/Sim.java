...
public class Sim  implements Comparator<Event>{
    ...
    public void PEC_init(){
        for (int i=0; i<ACO_.get_colony_size(); i++){
            ...
            AntMove ant_move = new AntMove(0, ant);
            PEC.add(ant_move);
        }

        for (int i=0; i<ACO_.get_n_edges(); i++) {
            evap = new Evaporation(ACO_.calc_time());
            Pec.add(evap);
        }
    }
...
//    		instant = ev.get_time_stamp();
    ...
    private void print_observations() {
        System. out. println("Observation " + obseervation_number + ":");
